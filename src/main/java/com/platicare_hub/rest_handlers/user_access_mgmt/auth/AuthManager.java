...
import com.platicare_hub.db_handlers.DatabaseResultsProcessors;
...
public class AuthManager
{
    ...
    public static String fetchAuthToken(String emailAddress)
    {
        ...
        try (Connection connection = DatabaseConnectionsHikari.getDbDataSource().getConnection())
        {
            ...
            if (resultSet != null)
            {
                ...
                storedToken = innerMap != null ? String.valueOf(resultMap.get("1").get("token")) : null;
            }
        }
        catch (Exception e)
        {
    ...
    public static void insertToken(String emailAddress, String token, String tokenType)
    {
        ...
        {
            sqlQuery = """
                    INSERT INTO user_auth_tokens (email, token)
                    VALUES (?, ?)
                    """;
        }
        ...
        {
           sqlQuery = """
                    INSERT INTO user_registration_tokens (email, token)
                    VALUES (?, ?)
                    """;
        }
        try (Connection connection = DatabaseConnectionsHikari.getDbDataSource().getConnection())
